...
declare
    ...
    l_response clob;
begin
    api_pkg.submit_transaction(:body_text, l_http_status, l_response);
    ords_util_pkg.emit_json(l_response, l_http_status);
end;
]'
    );
...
declare
    ...
    l_response clob;
begin
    api_pkg.validate_transaction(:body_text, l_http_status, l_response);
    ords_util_pkg.emit_json(l_response, l_http_status);
end;
]'
    );
...
declare
    ...
    l_response clob;
begin
    api_pkg.get_transaction(:actionRequestId, l_http_status, l_response);
    ords_util_pkg.emit_json(l_response, l_http_status);
end;
]'
    );
...
declare
    ...
    l_response clob;
begin
    api_pkg.resume_transaction(:actionRequestId, l_http_status, l_response);
    ords_util_pkg.emit_json(l_response, l_http_status);
end;
]'
    );
    ...
    begin
        ords.create_role('mfcs-integration-support');
    exception
        when others then
            if sqlcode <> -1 then
                null;
    ...
    l_roles(1) := 'mfcs-integration-support';
    l_patterns(1) := '/mfcs/v1/transactions/*/resume';
